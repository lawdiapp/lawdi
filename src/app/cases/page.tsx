...
import { cardSurface, iconTileAccent, pillButtonPrimary, statusPillSuccess } from "@/lib/theme";
    ...
    <main className="min-h-svh w-full overflow-x-hidden bg-background">
      <header className="border-b bg-background">
        <div className="mx-auto flex min-h-16 w-full max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <Link href="/dashboard" className="inline-flex min-h-10 items-center gap-2 rounded-full px-3 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Link href="/cases/new" className={cn(pillButtonPrimary, "min-h-11 gap-2 px-5 text-sm")}>
        ...
        <div className="mt-8">
          {error ? (
            <div className={cn(cardSurface, "py-8 text-center text-sm text-muted-foreground")}>
            ...
            <div className="grid gap-4">
              {cases.map((item) => (
                <Link
                  key={item.id}
                  href={`/cases/${item.id}`}
                  className={cn(cardSurface, "group grid min-w-0 gap-4 p-5 transition-shadow hover:shadow-md sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:p-6")}
                >
                  <div className="min-w-0">
                    <div className="flex min-w-0 flex-wrap items-center gap-2">
                      <h2 className="min-w-0 break-words text-lg font-semibold tracking-tight">{item.title}</h2>
                      <span className={cn(statusPillSuccess, "px-2.5 py-1 text-xs")}>{item.status}</span>
                    </div>
                    ...
                    {latestHearings.get(item.id)?.next_hearing_date ? (
                      <p className="mt-3 flex flex-wrap items-center gap-2 text-sm text-indigo-700">
            ...
            <div className={cn(cardSurface, "flex flex-col items-center gap-4 px-6 py-10 text-center sm:py-14")}>
              <span className={cn("grid size-14 place-items-center rounded-2xl", iconTileAccent)}>
              ...
              <Link href="/cases/new" className={cn(pillButtonPrimary, "mt-2 min-h-11 gap-2 px-5 text-sm")}>
                <Plus className="size-4" aria-hidden="true" />
                Add your first case
              </Link>
            </div>
